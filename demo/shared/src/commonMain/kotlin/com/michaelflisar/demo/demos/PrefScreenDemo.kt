...
import com.michaelflisar.demo.DateTimeUtil
import com.michaelflisar.demo.DispatcherIO
    ...
    Column {
        ...
        PreferenceScreen(
            ...
            PreferenceSection(
                ...
                PreferenceInfoExamples(showInfo)
                PreferenceThatNeedsDataStore(dataStore) { PreferenceBoolExamples(it, showInfo) }
                PreferenceButtonExamples(showInfo)
                PreferenceThatNeedsDataStore(dataStore) { PreferenceColorExamples(it) }
            }
            PreferenceSection(
                title = "Demos Section 2"
            ) {
                PreferenceThatNeedsDataStore(dataStore) { PreferenceDateExamples(it, showInfo) }
                PreferenceThatNeedsDataStore(dataStore) { PreferenceInputExamples(it) }
                PreferenceThatNeedsDataStore(dataStore) { PreferenceListExamples(it) }
                PreferenceThatNeedsDataStore(dataStore) { PreferenceNumberExamples(it) }
                PreferenceThatNeedsDataStore(dataStore) { PreferenceTimeExamples(it) }
                PreferenceDivider()
                    PreferenceThatNeedsDataStore(dataStore) { PreferenceDependenciesExamples(it) }
                PreferenceDivider()
                PreferenceCustomExamples(showInfo)
                        PreferenceThatNeedsDataStore(dataStore) { PreferenceCustom2Examples(it) }
            }
            PreferenceSection(
...
@Composable
private fun PreferenceScope.PreferenceThatNeedsDataStore(
    dataStore: IDemoDataStore?,
    content: @Composable PreferenceScope.(dataStore: IDemoDataStore) -> Unit
) {
    if (dataStore == null) {
        // skip... this one only works on "not wasm"
    } else {
        content(dataStore)
    }
}
    ...
    PreferenceSubScreen(
        ...
        PreferenceSection(
            ...
            PreferenceInfo(
                ...
                icon = { Icon(Icons.Outlined.Info, null) },
                onLongClick = {
                    showInfo("Hidden long press found :-)")
                }
            )
            PreferenceInfo(
...
private fun PreferenceScope.PreferenceDateExamples(
    ...
    PreferenceSubScreen(
        ...
        PreferenceSection(
            ...
            // end-snippet
            val date2 = dataStore.getLong("date2", now.toEpochDays())
                .collectAsState(initial = now.toEpochDays())
            PreferenceDate(
                value = date2.value.let { LocalDate.fromEpochDays(it) },
                onValueChange = {
                    scope.launch(DispatcherIO) {
...
private fun PreferenceScope.PreferenceDependenciesExamples(
    ...
    PreferenceSubScreen(
        ...
        PreferenceSection(
            ...
            // begin-snippet: demo-dependency2
            PreferenceInfo(
                title = "Sub Item 2.2...",
                subtitle = "Only visible if parent switch is enabled...",
                icon = { Icon(Icons.Default.Info, null) },
            ...
            // begin-snippet: demo-dependency3a
            ...
            // begin-snippet: demo-dependency3b
...
@Composable
fun PreferenceScope.PreferenceCustom2Examples(
    dataStore: IDemoDataStore
) {
    val scope = rememberCoroutineScope()
    ...
    PreferenceSubScreen(
        ...
        PreferenceSection(
            ...
            PreferenceAppSelector(dataStore)
        }
    }
}

@Composable
expect fun PreferenceScope.PreferenceAppSelector(dataStore: IDemoDataStore)
